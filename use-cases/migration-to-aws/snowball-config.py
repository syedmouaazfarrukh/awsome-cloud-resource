...
if __name__ == '__main__':
    ...
    response = snowball_client.list_jobs()
    snowball_job_id = response['JobList'][0]['JobId']
    
    if snowball_job_id is None:
        response = snowball_client.create_job(
            JobType='IMPORT',
            S3Bucket='mouaazodlmigrationtestbucket',
            SnowballCapacityPreference='T1',
            ShippingOption='STANDARD'
        )

        snowball_job_id = response['JobId']
